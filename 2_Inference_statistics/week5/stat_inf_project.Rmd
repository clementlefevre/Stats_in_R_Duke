...
```{r}
weapon_df <- gss %>% select(year,fear,owngun,sex,age,race,educ,coninc,partyid)
str(weapon_df)
```
...
weapon_df<- weapon_df %>% mutate(race_num=ifelse(race=='Black',0,ifelse(race=='White',2,1)))
 weapon_df<- weapon_df %>% mutate(partyid_num=as.numeric(partyid))                                
```
...
weapon_df_numeric<- weapon_df %>% select(-c(fear,sex,race,owngun,partyid))

str(weapon_df_numeric)
...
- very surprisingly to me, the party affiliation is almost not correlated with gun ownership.
