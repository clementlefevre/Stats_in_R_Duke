...
- for gun ownership, the most correlated variables are in order : race(white are more likely ot own guns than others and blacks $\rho$ = 0.4), sex (men are more likely to own guns), income (the more income, the more frequent is gunownership).
...
```{r}
p0<-ggplot(groupy_gun_race_male,aes(x=year,y=ratio,col=race)) + geom_line()

p1<-ggplot(groupy_gun_race_female,aes(x=year,y=ratio,col=race)) + geom_line()

p2<-ggplot(groupy_gun_race_male,aes(x=year,y=median_age,fill=race,color=race))+geom_line(alpha=.9)
p3<-ggplot(groupy_gun_race_female,aes(x=year,y=median_age,fill=race,color=race))+geom_line(alpha=.9)
