...
prediction1<-predict(model.AIC,newdata = ames_test)
mean(exp(prediction1))
...
RMSE_compute<- function(df,model){
  # Extract Predictions
prediction <- exp(predict(model, df))

# Extract Residuals
resid <- df$price - prediction
...
RMSE_compute(df = ames_train,model = model.full)
RMSE_compute(df = ames_train,model = model.AIC)
RMSE_compute(df = ames_test,model = model.full)
RMSE_compute(df = ames_test,model = model.AIC)
print('When predicting to ames_train, the RMSE for model.AIC is higher than the RMSE for model.full. However, when predicting to ames_test, the RMSE for model.full is higher.')
...
model.BIC<-stepAIC(model.full, k = log(nrow(ames_train)),trace=F)
