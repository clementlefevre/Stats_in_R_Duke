...
model2<-lm(log(price)~(log(area)+log(Lot.Area)+Bedroom.AbvGr+Overall.Qual+Land.Slope),data=ames_train)

model.AIC<-step(model1,trace = F,k = 2)
model.BIC<-step(model2,k=log(1000), trace = F)
...
model2<-lm(log(price)~log(area)+Bedroom.AbvGr,data=ames_train)
summary(model2)
ggpairs(ames_train,columns = c("price","area","Bedroom.AbvGr"))
...
model3<-lm(log(price)~log(area),data=ames_train)
df_resid<-data.frame(residuals =model3$residuals )

df_resid$Neighborhood<-ames_train$Neighborhood
head(df_resid)
df_resid %>% group_by(Neighborhood) %>% summarise(mean.res = mean(abs(residuals))) %>% top_n(3)
...
ames_train_qual <- ames_train %>%  select(price,Overall.Qual,Bsmt.Qual,Garage.Qual) 
sum(rowSums(is.na(ames_train_qual))>0)
...
model4<-lm(log(price)~ Overall.Cond+Overall.Qual,ames_train)
summary(model4)
prediction<-data.frame(prediction=model4$fitted.values)
prediction$price<-log(ames_train$price)
prediction$MS.SubClass<-ames_train$MS.SubClass
prediction %>% group_by(MS.SubClass) %>% summarise(med.pred = median(prediction)) %>% arrange(desc(med.pred))
...
ames_train_norm<-ames_train %>% filter(Sale.Condition=='Normal')
dim(ames_train_norm)
834/1000*100
...
summary(lm(log(price)~log(area),ames_train))
summary(lm(log(price)~log(area),ames_train_norm))
