...
On interesting festures might be the buildt surface vs total surface ratio :

```{r}
ames_train$Built.Ratio<- (ames_train$Kitchen.AbvGr+ames_train$Full.Bath)*ames_train$X2nd.Flr.SF/ ames_train$Bedroom.AbvGr


ggplot(ames_train,aes((Built.Ratio),price))+ geom_jitter(colour='orange2')+ geom_smooth(method = 'lm')
cor(as.numeric(ames_train$Built.Ratio),ames_train$price)
```
...
model1<-lm(price~Overall.Qual+Exter.Qual+area+Year.Built+Mas.Vnr.Area+Built.Ratio,data=ames_train)
summary(model1)
```
...
This model result in a RMSE of **33441.82$**.
* * *
...
```
The RMSE is **astoningshly lower**, which is **not was we would expect from a model**.
my guess is there are less outliers than in the training set, or my RMSE calculation got wrong.
...
model2<-lm(log(price)~Overall.Qual+Exter.Qual+log(area)+log(Lot.Area)+Year.Built+log(TotRms.AbvGrd+1)+log(X1st.Flr.SF)+BsmtFin.Type.1+Exterior.1st+Garage.Cars+Kitchen.Qual+Neighborhood,data=ames_train)
```
