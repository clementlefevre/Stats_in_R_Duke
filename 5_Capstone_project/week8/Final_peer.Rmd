...
How many are not informative (ie high NA ratio) ?
...
high.na.features
```
Ok, we can drop `Pool.QC`, `Misc.Feature`, `Alley`, `Fence` as they not provide enough info for the overall dataset.

Let's see which variables are not well balanced.
To proceed, we compute the skeweness for each numeric variable and select the top 20 most skewed :

```{r}

df_skew<-ames_train %>% select(-price,Pool.QC, Misc.Feature, Alley, Fence) %>% summarise_each(funs(skewness=skewness)) %>% gather()%>% arrange(desc(abs(value))) %>% top_n(20)
...
head(df_skewed_features_log)
```

And finaly weplot those logs values and see whether the variable distribution remains skewed :

```{r}
plot.vector<-vector(length=10)
...
W could plot a correlogramm of all variables :


```{r creategraphs}
lm1<- lm(price~.,data=ames_train)
