...
model1<-lm(price~Overall.Qual+Exter.Qual+area+Lot.Area+Year.Built+TotRms.AbvGrd+X1st.Flr.SF+BsmtFin.Type.1+Exterior.1st+Garage.Cars+Kitchen.Qual+Neighborhood,data=ames_train)
summary(model1)
```
We resulting model score quite well with an adjusted $\R^2 = 0.8612$. What is means ? It means the model manages to explains  84% of the overall variance, keeping in mind that a$\R^2 =1$ would result in explaining fully this variance.
...
model.AIC<-stepAIC(model1,k=2,trace=F)
model.BIC<-stepAIC(model1,k=log(nrow(ames_train)),trace=F)
