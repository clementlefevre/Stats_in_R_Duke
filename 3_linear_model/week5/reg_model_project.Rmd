...
As i have alread worked on this dataset for the Bayesian Inference project to predict the `audience_score`, this time i'd like to focus on the relationship between the title of a movie and its audience score. 

**More specifically, does the length of the title or its content improve the prediction of the audience score ?**
...
`audience_score` our target for the prediction is left-skewed, with a mean at 62%.
...
With the exception of an outlier in 1972, the title length does not follow a trend over the years.
...
All this process is well explained in the ["Analytical Edge MIT15071" online course](https://www.edx.org/course/analytics-edge-mitx-15-071x-3), that i can strongly recommend as a complement to the current course.
...
Just by curiosity again, how about the crew and directors ?

First, i combine all names and surnames into a single column :

```{r}

actors.cols<- grep('actor',colnames(movies),value = TRUE)
...
The title_length canbe ignored with **$\rho$=0.06**
...
- àll critics variables, much too correlated to the other scoring variables.
- `imdb_rating` ,  $\rho$ = 0.864 with `audience_score`, i considere it as a target variable also.
...
Now we have a lighter dataset, we can build a model and use the step function to select the best one :
...
Finally, both  crew_score  and title_score did pass the model selection with a p_value  below a reasonable significance level. Again, i did choose not using predictors too close to the target, i.e. of the rating/scoring family.
...
Good, the residuals are shaped along a slightly left skewed normal distribution, as the blue line for median residuals shows.
...
The previous observation is confirmed by the qqplot : the residuals follow a normal distribution.
...
```{r}
huntsman <- data.frame(genre = "Action & Adventure", runtime = 121, mpaa_rating = "PG-13",  critics_score = 17, best_any = "Yes",title_type="Feature Film", imdb_num_votes= 1000,best_pic_nom='yes',title_score=3,crew_score=4)

predict(best_lm,newdata = huntsman)
```
