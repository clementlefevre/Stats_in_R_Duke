...
variables<-c("at_bats","hits","wins","bat_avg", "new_onbase" ,"new_slug"  , "new_obs" )



compute.lm = function(var){
  
  m1<-lm(runs~get(var),data=mlb11)
  print(summary(m1)$r.squared)
  ret1<-ggplot(data = m1, aes(x = .fitted, y = .resid)) +
  geom_point() +
  geom_hline(yintercept = 0, linetype = "dashed") +
  xlab("Fitted values") +
  ylab("Residuals") + ggtitle(var)
  


ret2<-ggplot(data = m1, aes(x = .resid)) +
  geom_histogram(binwidth = 25) +
  xlab("Residuals")+ ggtitle(var)


ret3<-ggplot(data = m1, aes(sample = .resid)) +
  stat_qq()+ ggtitle(var)
  return(list("a"=ret1,"b"=ret2,"c"=ret3))
