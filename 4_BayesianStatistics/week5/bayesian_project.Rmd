...
ggcorrplot(corr, hc.order = FALSE, 
           ...
           ggtheme=theme_bw)
```
let's start with a Bayesian regression using all selected features :
...
We can now check whether the Markov chain Monte Carlo simulation has run long enough :
```{r}
...
```{r}
HPM = predict(movies.ZS, estimator="HPM")
